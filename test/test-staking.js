...
describe('Test Staking Smart Contract', function () {
  ...
  let token, staking
  let user, pool, feeReceiver
  before(async () => {
    [user, pool, feeReceiver] = await ethers.getSigners()
  ...
  beforeEach(async () => {
    ...
    staking = await StakingFactory.deploy(token.address, pool.address, feeReceiver.address)
  ...
  it("Stake, Harvest and Unstake Token", async () => {
    ...
    expect((await token.balanceOf(feeReceiver.address)).toString()).to.equal('0')
    ...
    expect((await token.balanceOf(feeReceiver.address)).toString()).to.equal('500')
    expect((await token.balanceOf(user.address)).toString()).to.equal('0')
    ...
    expect((await token.balanceOf(user.address))).to.equal('39000')
    expect((await token.balanceOf(feeReceiver.address))).to.equal('1000')
  })
  ...
  it("Stake and Unstake Token", async () => {
    ...
    infos = await staking.getStakingInfo(user.address)
    expect(infos[0].length).to.equal(0)
    expect((await token.balanceOf(user.address)).toString()).to.equal('39000')
    expect((await token.balanceOf(feeReceiver.address)).toString()).to.equal('1000')
  })
});
